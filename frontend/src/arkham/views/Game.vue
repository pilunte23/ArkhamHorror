...
const soundVolume = ref((() => {
  const value = Number(localStorage.getItem('arkhamSoundVolume'))
...
function handleSettingChange(event: Event) {
  ...
  if (detail?.key === 'arkhamTurnNotificationSoundDisabled') {
    turnNotificationSoundsDisabled.value = detail.value === 'true'
    return
  }
  if (detail?.key === 'arkhamEffectSoundsDisabled') {
    effectSoundsDisabled.value = detail.value === 'true'
    return
  }
  if (detail?.key === 'arkhamSoundVolume') {
    const value = Number(detail.value)
    if (!Number.isFinite(value)) return
    soundVolume.value = Math.max(0, Math.min(100, Math.round(value)))
  }
}
...
watch(activePlayerId, (newActivePlayerId, oldActivePlayerId) => {
  ...
  playAudioFile('turnIndicator.ogg', 'turn')
})
...
type AudioKind = 'turn' | 'effect'

function canPlayAudio(kind: AudioKind): boolean {
  if (soundsDisabled.value) return false
  if (kind === 'turn' && turnNotificationSoundsDisabled.value) return false
  if (kind === 'effect' && effectSoundsDisabled.value) return false
  return true
}

function playAudioFile(fileName: string, kind: AudioKind = 'effect') {
  if (!canPlayAudio(kind)) return
